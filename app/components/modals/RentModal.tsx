...
const RentModal = () => {
    ...
    const selectedPets = (value: string) => {
        if (selectedCategory.includes(value)) {
            setSelectedCategory(selectedCategory.filter((item) => item !== value));
        } else {
            setSelectedCategory((prevSelectedCategory) => [...prevSelectedCategory, value]);
        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[50vh] overflow-y-auto">
                {categories.map((item) => (
                    <div key={item.label} className="col-span-1">
                        <CategoryInput 
                            onClick={() => selectedPets(item.label)}
                            selected = {selectedCategory.includes(item.label)}
                            label = {item.label}
                            icon={item.icon}
                        />
                    </div>
